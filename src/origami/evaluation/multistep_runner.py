...
def run_multistep_suite(
    ...
    cases = [_load_multistep_scenario(path) for path in sorted(scenario_path.glob("*.yaml"))]
...
def _multistep_case_path(scenario_id: str, scenario_dir: Path | str) -> Path:
    safe_id = str(scenario_id).lower().strip().replace(" ", "_")
    if not safe_id:
...
def _run_multistep_case(case: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, step_config in enumerate(case["steps"]):
        if not isinstance(step_config, dict):
            raise ValueError(f"Multi-step scenario step must be a mapping: {scenario_id}[{index}]")
...
def _build_step_observation(
    ...
    if advance_position and previous_action is not None:
    ...
    if "at_s" in step_config:
        observation["mission_elapsed_s"] = float(step_config["at_s"])
...
def _evaluate_final_expected(
    ...
    checks = [
        _check_expected_value(key, expected_value, final_actual)
...
def _markdown_multistep_report(report: dict[str, Any]) -> str:
    lines = [
        ...
        "## Timeline Results",
        "",
        "| Scenario | Step | Result | At s | Final Move | STUM | Route | SEOM | Violations |",
        "| --- | --- | --- | ---: | --- | --- | --- | --- | --- |",
    ]
    for scenario in report["scenarios"]:
        for step in scenario["steps"]:
            ...
            lines.append(
                ...
                f"{step.get('at_s') if step.get('at_s') is not None else '-'} | "
